...
In your distribution, add a `XT::Files` configuration file:
...
files under `xt` that iterate over a `check_file` function of the author
test.
...
differently, you have to load it in every `.t` file
...
configuration which is the `XT::Files` config file or, if that does not
exist, the [XT::Files::Plugin::Default](https://metacpan.org/pod/XT::Files::Plugin::Default) plugin.
...
The `plugin` method used [Module::Load](https://metacpan.org/pod/Module::Load) to load the plugin. If a VERSION
...
it then calls the plugins `new` method and passes `$self` and expects an
...
## Example 1 Use a test that supports `XT::Files` with default config files
